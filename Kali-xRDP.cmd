...
%GO% "cd /tmp ; dpkg --purge --force-all locales-all ; DEBIAN_FRONTEND=noninteractive apt-get download kali-archive-keyring libc-bin libc-l10n libc6 libpam0g locales-all libcrypt1 libgcc-s1 libstdc++6 libpam-runtime libpam-modules-bin libpam-modules gcc-13-base libzstd1 ; DEBIAN_FRONTEND=noninteractive dpkg -i --force-all ./*.deb 2> /dev/null; rm ./*.deb ; DEBIAN_FRONTEND=noninteractive apt-get -y --no-install-recommends install git gnupg2 libc-ares2 libssh2-1 libaria2-0 aria2 acl pciutils ; echo 'exit 0' > /usr/bin/lspci ; echo 'exit 0' > /usr/bin/setfacl ; rm -rf %GITPRJ% ; echo 'Clone Git repo...' ; git clone --quiet -b %BRANCH% --depth=1 https://github.com/%GITORG%/%GITPRJ%.git ; chmod +x /tmp/Kali-xRDP/dist/usr/local/bin/* ; cp -p /tmp/Kali-xRDP/dist/usr/local/bin/systemd-sysusers /usr/local/bin ; apt-get -fy install systemd --no-install-recommends ; cp -p /tmp/Kali-xRDP/dist/usr/local/bin/systemd-sysusers /usr/bin ; apt-get -fy install" > "%TEMP%\Kali-xRDP\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% Prepare Distro.log" 2>&1 
